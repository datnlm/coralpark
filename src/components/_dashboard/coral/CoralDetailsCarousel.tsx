...
type CoralDetailsCarouselProps = {
  coral: Coral;
};

export default function ProductDetailsCarousel({ coral }: CoralDetailsCarouselProps) {
  ...
  const imagesLightbox = coral.images.map((_image: any) => _image);
  ...
  const settings2 = {
    ...
    variableWidth: true,
    centerPadding: '0px',
    slidesToShow: coral.images.length > 3 ? 3 : coral.images.length
  };
    ...
    <RootStyle>
      ...
      <Box
        sx={{
          ...
          ...(coral.images.length === 1 && { maxWidth: THUMB_SIZE * 1 + 16 }),
          ...(coral.images.length === 2 && { maxWidth: THUMB_SIZE * 2 + 32 }),
          ...(coral.images.length === 3 && { maxWidth: THUMB_SIZE * 3 + 48 }),
          ...(coral.images.length === 4 && { maxWidth: THUMB_SIZE * 3 + 48 }),
          ...(coral.images.length >= 5 && { maxWidth: THUMB_SIZE * 6 }),
          ...(coral.images.length > 2 && {
            position: 'relative',
            '&:before, &:after': {
              top: 0,
              zIndex: 9,
      ...
      >
        <Slider {...settings2} asNavFor={nav1} ref={slider2}>
          {coral.images.map((item: any) => (
            <ThumbnailItem key={item.imageUrl} item={item.imageUrl} />
          ))}
        </Slider>
      </Box>
