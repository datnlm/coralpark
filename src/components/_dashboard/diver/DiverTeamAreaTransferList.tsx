...
type DiverTeamAreaTransferListProps = {
  isEdit: boolean;
...
export default function DiverTeamAreaTransferList({
  isEdit,
  currentDiverTeam,
  areaList,
  submitRef,
  onSubmitCallback
}: DiverTeamAreaTransferListProps) {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      diverTeam: currentDiverTeam || null,
      area: [{}]
    },
  ...
  useEffect(() => {
    ...
    currentDiverTeam?.areas.map((v: Area) => listSelectedAreaId.push(Number(v.id)));
    const mapId: number[] = [];
